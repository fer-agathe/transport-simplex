...
We can also visualize the estimation of density of the Dirichlet distributions fitted on the dataset (using {VGAM}). First, 
```{r define-dir_0}
dir_0 <- VGAM::vglm(
  cbind(A,B,C)  ~ 1,
  VGAM::dirichlet,
...
VGAM::Coef(dir_1)
```
We prepare a grid to visualize the estimated densities.
```{r define-contour_df}
u <- (0:100) / 100
u <- u[-c(1, length(u))]
x <- expand.grid(u, u)
...
The estimated densities are shown in @fig-ternary-toydataset-densities.
